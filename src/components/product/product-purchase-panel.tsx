  ...
  return (
    <>
      <div className="flex flex-col gap-3 md:gap-4 lg:col-span-3">
        <div className="relative aspect-square w-full overflow-hidden rounded-2xl border border-zinc-200 bg-[#FFFFFF]">
          <Image
          ...
          {galleryImages.map((image, index) => {
            ...
            return (
              <button
                ...
                className={`relative aspect-square overflow-hidden rounded-xl border bg-[#FFFFFF] transition-all ${isActive ? "border-[#D4AF37] opacity-100 ring-1 ring-[#D4AF37]/40" : "border-zinc-200 opacity-80 hover:border-[#D4AF37] hover:opacity-100"}`}
              >
                <Image
                  src={image.url}
                  alt={image.alt}
      ...
      <div className="flex h-fit w-full flex-col rounded-2xl border border-zinc-200 p-4 md:p-6 lg:sticky lg:top-24 lg:col-span-2 lg:p-7">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          {hasDiscount ? (
            <span className="inline-flex h-8 items-center rounded-md bg-[#D4AF37] px-3 text-[12px] font-medium font-serif tracking-wide text-[#111111]">
              DESCONTO
            </span>
          ) : null}
          {hasFreeShipping ? (
            <span className="inline-flex h-8 items-center rounded-md bg-emerald-600 px-3 text-[12px] font-medium font-serif tracking-wide text-white">
              FRETE GRÁTIS
            </span>
          ) : null}
        </div>
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <h1 className="text-[24px] font-medium font-serif leading-[1.2] tracking-tight text-[#111827]">{product.name}</h1>
          <FavoriteButton productId={product.id} className="h-10 w-10 shrink-0" />
        </div>

        <div className="mb-5 flex w-full items-center justify-between border-b border-zinc-200 pb-3 text-[13px] text-[#666666]">
          <span>
            Código: <strong className="text-[#1A1A1A]">{sku}</strong>
          </span>
          <span>
            Marca:{" "}
            {product.brandSlug ? (
              <Link href={`/marca/${product.brandSlug}`} className="font-medium font-serif text-[#1A1A1A] transition-colors hover:text-[#D4AF37]">
                {brandLabel}
              </Link>
            ) : (
              <strong className="text-[#1A1A1A]">{brandLabel}</strong>
            )}
          </span>
        </div>
        ...
        <div className="mb-6 flex flex-col">
          ...
          <div className="flex flex-col">
            <span className="mb-3 text-[30px] font-medium font-serif leading-none tracking-tight text-[#1A1A1A]">
              {formatCurrency(product.price)}
            </span>

            <div className="flex items-center gap-2">
              <PixIcon className="h-[18px] w-[18px] text-[#32BCAD]" />
              <span className="text-[16px] font-medium font-serif text-[#1A1A1A]">{formatCurrency(pixPrice)} no Pix</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 flex flex-col gap-3">
          ...
          <Link
            ...
            target="_blank"
            rel="noreferrer"
            className="flex h-[50px] w-full items-center justify-center gap-2 rounded-[10px] border border-[#25D366] bg-[#25D366] text-[16px] font-medium font-serif text-white transition-colors hover:border-[#22c55e] hover:bg-[#22c55e]"
          >
        ...
        <div className="mt-5 flex items-center gap-3 border-t border-zinc-100 pt-5">
          <span className="text-[15px] font-medium font-serif text-[#111827]">Compartilhar:</span>
