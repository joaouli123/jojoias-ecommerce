  ...
  {/* Actions - Desktop */}
        <div className="hidden lg:flex items-center gap-8 shrink-0">
          ...
          <div className="relative group/atendimento z-50">
            <Link href="/atendimento" className="flex items-center gap-2.5 hover:opacity-80 transition-opacity pb-8 -mb-8">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-wide leading-tight mb-0.5">Atendimento</span>
                <span className="text-[13px] font-bold text-zinc-800 leading-none">Central de Ajuda</span>
              </div>
            </Link>
            ...
            <div className="absolute right-0 top-[100%] mt-6 opacity-0 invisible group-hover/atendimento:opacity-100 group-hover/atendimento:visible transition-all duration-300">
              <div className="w-[320px] bg-white rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-zinc-100 p-5 flex flex-col relative before:content-[''] before:absolute before:-top-2 before:right-12 before:w-4 before:h-4 before:bg-white before:border-t before:border-l before:border-zinc-100 before:transform before:rotate-45">
                <div className="flex flex-col gap-4">
                  <Link href={settings.whatsappUrl} target="_blank" rel="noreferrer" className="flex gap-3 items-center rounded-2xl transition-colors hover:bg-zinc-50 -mx-2 px-2 py-1.5">
                    <MessageCircle className="w-5 h-5 text-zinc-900 shrink-0" />
                    <div className="flex flex-col">
                      <span className="text-[13px] font-bold text-zinc-900 leading-none mb-1">WhatsApp:</span>
                      <span className="text-[14px] text-zinc-600 leading-none">{settings.whatsappPhone}</span>
                    </div>
                  </Link>
                  <div className="w-full h-px bg-zinc-100"></div>
                  <Link href={`tel:${settings.supportPhone.replace(/\D/g, "")}`} className="flex gap-3 items-center rounded-2xl transition-colors hover:bg-zinc-50 -mx-2 px-2 py-1.5">
                    <Phone className="w-5 h-5 text-zinc-900 shrink-0" />
                    <div className="flex flex-col">
                      <span className="text-[13px] font-bold text-zinc-900 leading-none mb-1">Telefone:</span>
                      <span className="text-[14px] text-zinc-600 leading-none">{settings.supportPhone}</span>
                    </div>
                  </Link>
                  <div className="w-full h-px bg-zinc-100"></div>
                  ...
                  <div className="flex gap-3 items-center">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[13px] font-bold text-zinc-900 leading-none mb-1">Horário de Atendimento</span>
                      <span className="text-[14px] text-zinc-600 leading-none">{settings.businessHours}</span>
                    </div>
                  </div>
                </div>
                <div className="mt-5 border-t border-zinc-100 pt-5">
                  <Link href="/atendimento" className="block w-full rounded-[20px] bg-[#111111] py-3.5 text-center text-[14px] font-bold text-white transition-colors hover:bg-zinc-800">Fale Conosco</Link>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="relative group/user z-50">
            <Link href="/login" className="flex items-center gap-2.5 hover:opacity-80 transition-opacity pb-8 -mb-8">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-wide leading-tight mb-0.5">Bem-vindo(a)</span>
                <span className="text-[13px] font-bold text-zinc-800 leading-none">Entre <span className="font-normal text-zinc-500">ou</span> Cadastre-se</span>
              </div>
            </Link>
            ...
            <div className="absolute right-0 top-[100%] mt-6 opacity-0 invisible group-hover/user:opacity-100 group-hover/user:visible transition-all duration-300">
              <div className="w-[300px] bg-white rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-zinc-100 p-5 flex flex-col relative before:content-[''] before:absolute before:-top-2 before:right-12 before:w-4 before:h-4 before:bg-white before:border-t before:border-l before:border-zinc-100 before:transform before:rotate-45">
                <Link href="/login" className="flex w-full items-center justify-between rounded-[20px] bg-[#111111] px-5 py-3.5 text-[15px] font-bold text-white transition-colors hover:bg-zinc-800">
                  Entrar <ChevronRight className="w-5 h-5" />
                </Link>
                <div className="flex flex-col mt-4 gap-2.5 px-1">
                  <Link href="/account/orders" className="text-[15px] font-medium text-zinc-800 hover:text-[#D4AF37] transition-colors">Meus pedidos</Link>
                  <div className="text-[15px] text-zinc-800">
                    Cliente novo? <Link href="/register" className="font-bold border-b-2 border-[#111111] hover:text-[#D4AF37] hover:border-[#D4AF37] transition-colors pb-0.5">Cadastre-se</Link>
                  </div>
                </div>
      ...
      <nav className="hidden lg:block w-full border-y border-zinc-200 bg-white">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
          <ul className="flex items-center gap-0 h-14 text-[15px] font-bold text-zinc-900">
            ...
            {navigationItems.map((item, index) => {
              ...
              return (
                ...
                >
                  <Link
                    href={item.href}
                    className="flex items-center gap-2.5 h-full px-5 rounded-lg text-zinc-900 hover:text-[#D4AF37] hover:bg-[#D4AF37]/[0.08] transition-all"
                  >
            ...
            <li className="ml-auto pl-6">
              <Link href="/categoria/ofertas" className="inline-flex items-center gap-2 rounded-xl bg-[#111111] px-5 py-2 text-[14px] font-bold text-[#D4AF37] hover:bg-[#1a1a1a] transition-colors shadow-sm">
