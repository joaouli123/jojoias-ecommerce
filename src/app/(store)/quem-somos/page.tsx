    ...
    <div className="bg-white">
      ...
      <section className="mx-auto grid max-w-5xl gap-10 px-6 py-16 sm:px-8 lg:grid-cols-[1.3fr_0.7fr] lg:px-10 lg:py-20">
        <div className="rounded-[20px] border border-zinc-200 bg-white p-8 shadow-sm">
          <h2 className="text-2xl font-medium font-serif text-[#1A1A1A]">Nossa essência</h2>
          <p className="mt-5 whitespace-pre-line text-base leading-8 text-[#666666]">{settings.aboutContent}</p>
        </div>
        ...
        <div className="space-y-4">
          {highlights.map((item) => {
            ...
            return (
              <div key={item.title} className="rounded-[20px] border border-zinc-200 bg-[#FFFFFF] p-6">
                <div className="flex h-12 w-12 items-center justify-center rounded-[20px] bg-[#1A1A1A] text-[#D4AF37]">
                  <Icon className="h-5 w-5" />
                </div>
                <h3 className="mt-4 text-lg font-medium font-serif text-[#1A1A1A]">{item.title}</h3>
                <p className="mt-2 text-sm leading-7 text-[#666666]">{item.description}</p>
              </div>
            );
          })}
        </div>
